...
// The tag from the URL MUST be in this list.
...
app.get('/', (req, res) => {
  ...
  if (tag && VALID_TAGS.includes(tag)) {
    // Case 1: Valid tag from QR code (?tag=gs1)
    // Show the contact form immediately.
    res.send(getContactFormPage(tag));
  } else {
    // Case 2: No tag or invalid tag (typed in URL)
    // Show the landing page to look up the tag.
...
app.post('/lookup', (req, res) => {
  ...
  // Redirect to the GET / route with the tag in the query
  // This will then be validated by the logic in app.get('/')
